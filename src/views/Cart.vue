                  ...
                  <table>
                    ...
                    <tbody v-if="keranjangUser.length>0">
                      <tr v-for="keranjang in keranjangUser" :key="keranjang.id">
                        ...
                        <td class="p-price first-row">${{keranjang.harga}}</td>
                        <td class="delete-item" @click="removeProduct(keranjang.id)">
                          <a href="#">
...
export default {
  ...
  methods: {
    removeProduct(idP) {
      ...
      let index = itemKeranjangUserStorage.findIndex(id => id == idP);
      this.keranjangUser.splice(index, 1);
      const parsed = JSON.stringify(this.keranjangUser);
      localStorage.setItem("keranjangUser", parsed);
    },
    checkOut() {
      let idP = this.keranjangUser.map(function(product) {
        return product.id;
      });
  ...
  computed: {
    ...
    totalBiaya() {
      return (this.totalHarga * 110) / 100;
    }
  }
};
</script>